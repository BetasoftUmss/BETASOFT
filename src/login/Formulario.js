...
const baseUrl = "https://app-betaweb.herokuapp.com/api/usuario";
          ...
          for (var i = 0; i < response.length; i++) {
            if (
              ...
              if (respuesta.instructor == null && respuesta.admin == null) {
                ...
                cookies.set(
                  "id_estudiante",
                  respuesta.estudiante.id_estudiante,
                  { path: "/" }
                );
                cookies.set("nombre", respuesta.estudiante.nombre, {
                  path: "/",
                });
                cookies.set(
                  "apellido_paterno",
                  respuesta.estudiante.apellido_paterno,
                  { path: "/" }
                );
                cookies.set(
                  "apellido_materno",
                  respuesta.estudiante.apellido_materno,
                  { path: "/" }
                );
                ...
                respuesta.instructor == null &&
                respuesta.estudiante == null
              ) {
                cookies.set("instructor", respuesta.instructor, { path: "/" });
                cookies.set("estudiante", respuesta.estudiante, { path: "/" });
                cookies.set("id_admin", respuesta.admin.id_administrador, {
                  path: "/",
                });
                cookies.set("nombre", respuesta.admin.nombre, { path: "/" });
                cookies.set(
                  "apellido_paterno",
                  respuesta.admin.apellido_paterno,
                  { path: "/" }
                );
                cookies.set(
                  "apellido_materno",
                  respuesta.admin.apellido_materno,
                  { path: "/" }
                );
                ...
                cookies.set(
                  "id_instructor",
                  respuesta.instructor.id_instructor,
                  { path: "/" }
                );
                cookies.set("nombre", respuesta.instructor.nombre, {
                  path: "/",
                });
                cookies.set(
                  "apellido_paterno",
